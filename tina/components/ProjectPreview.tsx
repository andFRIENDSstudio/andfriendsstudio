...
export const ProjectPreview = (props: any) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const cursorRef = useRef<HTMLDivElement>(null);
  const textElementRef = useRef<HTMLDivElement>(null);
  const textWrapperRef = useRef<HTMLDivElement>(null);
  
  // Access parent form values through tinaForm
  const form = props.tinaForm;
  const values = form?.values || {};
  
  // Navigate to the current project in the array
    ...
    <div style={{ marginTop: '1rem', marginBottom: '2rem' }}>
      ...
      >
        ...
        >
          ...
          >
            <div ref={textWrapperRef} style={{ width: '100%', overflow: 'hidden' }}>
              <div ref={textElementRef} style={{ display: 'inline-block', whiteSpace: 'nowrap' }} />
            </div>
          </div>
        </div>
